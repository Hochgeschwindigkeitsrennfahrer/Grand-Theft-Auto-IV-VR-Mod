...
2. **Phase A used OpenXR:** SteamVR OpenXR often has **no usable 32-bit runtime** → WMR OpenXR there. **Phase B does not need that:** we use **OpenVR** (SteamVR), like L4D2VR/HL2VR. Do not confuse the two.
3. **Neither OpenVR nor OpenXR accepts raw D3D9 textures.** HL2/L4D2 submit **Vulkan** from inside/near DXVK — not “Vulkan magically = VR.”
4. **Stock DXVK / Steam “Vulkan guide”** = FPS only. **VR** needs a **fork** with compositor hooks (L4D2VR, IronWolf, HL2VR custom `d3d9.dll`).
...
11. Reverb G2 + OpenVR ⇒ **SteamVR** running (same as other OpenVR mods).

## Useful Phase A artifacts to reuse conceptually

| Phase A | Phase B use |
|---------|-------------|
| `docs/OPENXR_WIN32.md` | Phase A only; Phase B uses OpenVR |
| `docs/HL2VR_PATH.md` | This *is* that path (OpenVR + DXVK) |
