...
> **2026-07-27 ~16:48 CLEAN Mode 136 LATE-LATCH:** Live worktree = **`gtaiv-dxvk-vr-clean`**. stereo=**136** · dualn=**2** · buildid=`20260727-mode136-latelatch`. Mode120 content + `Submit_TextureWithPose` late-latch. OpenVR **2.12.14**. Kill=**120**. See clean `docs/CURRENT-STATE.md` + `SESSION_PREP_20260727.md`.

> **2026-07-27 ~03:51 CLEAN Mode 121 redeploy:** Stale clean ASI was OpenVR **2.15.6** → rebuilt against **v2.12.14** (`IVRSystem_023`). Deployed clean ASI + openvr to game. **stereo=`121`** · dualn=`3` · buildid=`20260727-clean-121-redeploy`. FusionFix SkipMenu kept. MAIN Mode88 ASI backed up at `_vr_glue_backup_clean121_redeploy_20260727-035100`. Compare vs prior MAIN 88.

> **2026-07-27 ~03:40 Chooser off again:** Restored `plugins\GTAIV.EFLC.FusionFix.asi` + `.ini`/`.cfg` from `_vr_glue_backup_main_abtest_20260727-032018` with **`SkipMenu=1` / `SkipIntro=1`**, `FieldOfView=0`. `update\` untouched. Main ASI (`20260727-openvr212-fix105`) untouched. Stereo file **left alone** (was no longer 88 — live value at restore time). **Pre-MAIN LKG:** clean Mode **120** (dualn=2, smooth 90) then experiment **121** (dualn=3); MAIN switch = Mode **88** `20260727-main-abtest-f5abc06`. Sidecars: `gtaiv-dxvk-vr-clean\out-asi\gtaiv_dxvk_vr.mode120-lkg.asi` / `.mode121.asi` and backup `_vr_glue_backup_main_abtest_20260727-032018\gtaiv_dxvk_vr.asi.mode120-lkg` / `.mode121`.

> **2026-07-27 ~03:36 VR-start fix (post-reboot):** Symptom **„VR startet nicht“** — log `OpenVR VR_Init FAILED: Interface Not Found (105)`. Deploy inventory was OK (ASI/d3d9/dinput8/openvr/stereo=`88`). Root cause: ASI built against OpenVR SDK **2.15.6** (`IVRSystem_026`) while installed SteamVR is **2.12.14** (vrclient only up to **`IVRSystem_023`**). **Fixed:** checked out `thirdparty/openvr` **v2.12.14**, rebuilt ASI, deployed ASI + matching `openvr_api.dll` 2.12.14. Proof: `VR_Init OK`, `StereoMode: 88`, `StereoSubmit L=1 R=1`, `StereoDiff absdiff≠0`. buildid=`20260727-openvr212-fix105`. **Do not bump OpenVR past installed SteamVR** without re-checking `IVRSystem_*` in `vrclient.dll`. Kill=**45**. Game running — put headset on / CE chooser → play.

> **2026-07-27 ~03:21 MAIN A/B deploy (post-reboot):** Nuked all `gtaiv_dxvk_vr.*` + Mode120/121 ASI backups + **plugins FusionFix.asi/ini/cfg**. Kept `update\` (incl. FusionFix asset tree — **no ASI load without plugins ASI**). Kept `d3d9.dll` / `dxvk.conf` / `dinput8.dll`. Built **main** `f5abc06` (+ `vr_display.h` FOVPROOF decls fix so Mode88 compiles). Deployed ASI + `openvr_api.dll`. **stereo=`88`** · dualn=`2` · eyert=`1440` · buildid=`20260727-main-abtest-f5abc06`. **No clean-tree Mode 120/121.** Backup: `Documents\cursor\_vr_glue_backup_main_abtest_20260727-032018`. Expect **CE chooser** (FusionFix SkipMenu gone). Compare vs clean Mode120 after cold start. Kill=**45**. *(This build hit OpenVR 105 until ~03:36 fix.)*

> **2026-07-27 ~02:30 Chooser fix (real cause):** Stock CE **always** shows Rockstar intro → DLC/main select without **FusionFix Skip Menu**. Earlier `update\` restore alone was **not** enough — `FusionFix.asi` was still missing after fullnuke. **Restored** `plugins\GTAIV.EFLC.FusionFix.asi` + `.ini`/`.cfg` from `_vr_glue_backup_fullnuke_20260727-015242` with **`SkipMenu=1` / `SkipIntro=1`**, `FieldOfView=0`. Proof: DirectExe launched **`GTAIV.exe` only** (no `PlayGTAIV`); modules show FusionFix + `gtaiv_dxvk_vr.asi`; our DXVK `d3d9.dll` unchanged. Stereo still **`120`**. Start: Desktop **GTA IV Quick Restart**.

> **2026-07-27 ~02:25 Launch note (partial):** Restored `update\` FusionFix content tree after fullnuke emptied it. That alone did **not** skip the CE chooser (see ~02:30).

> **2026-07-27 ~02:00 Mode51 ~50 FPS probe (virgin `01f355b`):** Game dir **clean** (FusionFix/plugins/update gone; `dxvk.conf` = only hideNvidiaGpu + pipelineLibrary off — **no** FPS limit). Log: `ASI_BUILD_ID 20260727-github-fresh-01f355b`, `StereoMode: 51`, `STEREO-AER`, StretchRect pair-hold ~2 frames/pair, Submit L=R=1. **No dirty leftover.** App Present ≈**41/s** (2640 MonoSubmit / ~64s log life) — real ~40–50, not overlay lie. Mode51 AER does **not** hard-cap Present at 45; temporal+WaitGetPoses often **misses 90Hz → ~45 lock**; SteamVR `motionSmoothingOverride:1` fills HMD to 90. NVIDIA **desktop** overlay ≈ App Present — **do not expect 90** there on Mode51 when half-locked. **Next A/B:** stereo **`0`** already written — cold restart; if mono ≈90 → Mode51 cost/half-lock; if mono also ~50 → elsewhere. Optional lighter: stereo **`50`** (no TextureWithPose).

> **Active VR worktree:** `gtaiv-dxvk-vr-clean` — see that tree’s `docs/CURRENT-STATE.md` for live status. 
> **2026-07-27 PERF REALRESTORE:** literal `mode110-lookfix-pre111.asi` deployed to game (stereo **110**, hash `A5AFDA1B…`). HUD strip did **not** fix Mode 111; deeper cost = Praydog DrawScene dual + true-FOV overfill. **Cold start** required.
...
Full write-up: **`(removed)`** (GTA5 + CP2077 mode matrix).
...
- **Mode74 implication:** our AER + pose Submit **is** the thirdparty-vr analog. Dual×2 is *our* Halo-class experiment — has nothing to port there.
