...
## 2026-08-03 — HUD stack + real Full restart + release (hud4)

**buildId:** `20260803-909-hud4` · **stereo:** `909`

### HUD
- Stack like stock screenshot: **Money → Wanted stars → Ammo → Weapon** (top→bottom), X≈0.68 (toward center)
- Radar moved inward: X≈0.30 (was corner-ish)
- ASI no longer rewrites `hud.dat` / live-pokes / radiohud / crosshair size scan (those crashed)

### Full restart
- Menu **Full restart** now ShellExecutes `FULL-RESTART.bat` and quits the process (not just file re-read)
- Ship `QUICK-START.bat` + `FULL-RESTART.bat` in the release zip

### Test
1. Close GTA if open → run `QUICK-START.bat` or Steam Play  
2. Log: buildId `20260803-909-hud4`, no `radiohud Y` / `crosshair size slots`  
3. Check money/stars/ammo/weapon stack + radar more centered  
4. F3 → Full restart → game should quit and relaunch via Steam  

### Release
- `dist/gtaiv-dxvk-vr-release.zip` + GitHub Release on `master`

---
## 2026-08-03 — Crash hotfix (hud3 → hud3fix)

**buildId:** `20260803-909-hud3fix` · **stereo:** `909`  
**Cause:** hud3 ASI (1) rewrote `radiohud.dat` into glued floats (`0.100000.07600`) → crash; (2) blind crosshair-size mem scan (`ScanCrosshairSizesOnce`) poked unrelated floats.

**Fix (binary patch on hud3 ASI — do not rebuild from current tree yet):**
- `RET` at `ScanCrosshairSizesOnce` (`0x46BA0`)
- neutralize radiohud `WriteWholeFile` call (`0x456B9` → `xor eax,eax`)
- restored `radiohud.dat` + `hud.dat` from `.gtaiv-dxvk-vr.bak`; radiohud set read-only

**Test:** SteamVR → GTA → Quick Restart. Log must **not** show `HudLayout: radiohud Y` or `crosshair size slots`. Game should stay up.  
**Do not** run a full source rebuild until Mode 909 `stereo_render.cpp` is recovered (tree was overwritten with AER-era file).

---
## 2026-08-02 — Mode 905 v2 (dual-end + render pose) — fusion/jump fix

**buildId:** `20260802-mode905-dualend-pose` · **stereo:** `905`

### Headset result (v1)
Truthful EndScene skip **fixed** look-around latency/smoothness. Side effects: **no fusion** + **frames jumping**.

### Cause
v1 submitted at the *next* dual’s mid-draw EndScene with a **fresh late-latch pose** on the **previous** pair (and late-latch was overwriting render-pose staging). Smooth rate, wrong pose/pair → swim + jump.

### v2 fix (same mode 905)
- `WaitGetPoses` at **dual start** (canonical)
- Submit **matched L+R once at dual end** with **true render pose**
- EndScene does **not** submit (Motion Smoothing still sees ~45 content fps)
- Late-latch no longer overwrites render-pose modes

### Test
1. Motion Smoothing ON → Quick Restart after deploy  
2. Log: `Mode905: … dual end … RENDER POSE`, `Mode905: EndScene defers to dual-end`, `RateAudit:`  
3. Expect: keep v1 smoothness, **restore fusion**, less jumping  
4. Kill: `903`

---
## 2026-08-02 — Mode 905 deployed (truthful submit + RateAudit)

**Branch:** `claude/mode-244-render-pose`  
**buildId:** `20260802-mode905-truthful`  
**stereo:** `905`

### Why (after 1440² felt the same)
Resolution cut did not help → bottleneck is structural (2 EndScenes / dual), not fill-rate.  
Mode **905** = Mode **903** (H2+H4+Direct) + **truthful submit**: only `WaitGetPoses`+Submit when a dual produced new content. Duplicate EndScene is skipped (input still runs). SteamVR can then Motion-Smooth ~45→90.

### SteamVR (required for this test)
**Settings → Video → Motion Smoothing = ON** (was off in earlier tests).

### Log lines
- `Mode905: Mode903 + TRUTHFUL SUBMIT...`
- `Mode905: TRUTHFUL SUBMIT armed`
- `Mode905: skip duplicate EndScene submit`
- `RateAudit: duals/s=… EndScene/s=…` — expect ~45 duals and ~90 EndScene if half-rate theory holds
- `FrameTiming: clientIntervalMs=…` — ~22 ms client interval with smoothing is OK; ~11 ms with duplicate submits was the old lie

### Headset checks
1. Motion Smoothing ON → start GTA → confirm buildId + Mode905.
2. Look around — less smear than 903 if theory holds.
3. Kill: stereo `903` or `243`.

### 900 family quick map
| Mode | Extra vs 243 |
|------|----------------|
| 903 | H2+H4+Direct |
| 904 | + zero-copy mid-dual |
| **905** | 903 + truthful submit |

---
## 2026-08-02 — Mode 903 deployed (Direct 1:1 + H2/H4 on Mode 243)

**Branch:** `claude/mode-244-render-pose`  
**buildId:** `20260802-mode903-direct`  
**stereo:** `903` (deployed)

### 900 family (Mode 243 late-latch base)
| Mode | Meaning |
|------|---------|
| **900** | H2 rolled IPD only |
| **901** | H4 eye-center 6DoF only |
| **902** | H2 + H4 |
| **903** | 902 + **DIRECT** 1:1 BB→eye + Submit `VRTextureBounds` (no canvas resample; EndScene pair submit) |
| **904** | 902 + **ZERO-COPY** mid-dual BB Submit per eye (compiled in; not deployed — risk of eye drag) |

Kill: `902` / `243` / `0` in `gtaiv_dxvk_vr.stereo`, Quick Restart.

### Log lines (903)
- `Mode903: Mode243 + H2+H4 + DIRECT 1:1...`
- `capture path=DIRECT 1:1 BB` (or `canvas fallback` if measTan < cover)
- `DIRECT Submit bounds eye=...`
- `H2 ROLLED IPD` / `H4 eye-center 6DoF`

### Headset checks (903)
1. SteamVR → GTA → confirm buildId + Mode903 in log.
2. Distance sharpness vs **902** (should look closer to mono).
3. Heaviness / look-around blur vs 902 (2× scene still there).
4. If world looks “massive” / stretched: check `fpfov` not read-only at 90 — Direct falls back to canvas.
5. When ready for risk test: set stereo=`904`, Quick Restart — look for `Mode904: ZERO-COPY mid-dual`.

### Not changed
IPD tune-by-feel; Mode 268 number unused (903/904 used instead).

---
## 2026-08-02 — Mode 902 deployed (H2+H4 on Mode 243 base)

**Branch:** `claude/mode-244-render-pose`  
**buildId:** `20260802-mode902-h2h4-243`  
**stereo:** `902` (superseded by 903)

### What changed (camera only — Mode 243 submit path)
| Mode | Meaning |
|------|---------|
| **900** | Mode **243** + **H2** rolled IPD baseline (view stays leveled; L/R sep follows yaw-coupled HMD right / roll) |
| **901** | Mode **243** + **H4** eye-center **6DoF** (HMD lean / neck-arc on ped pivot) |
| **902** | **H2 + H4** together |

Kill: write `243` (or `0`) into `gtaiv_dxvk_vr.stereo`, Quick Restart.

### Log lines to confirm membership
- `Mode902: Mode243 + H2 rolled IPD + H4...`
- `H2 ROLLED IPD baseline ON`
- `H4 eye-center 6DoF ON`
- Must stay on late-latch (243), **not** Mode 244 render-pose path.

### Headset checks
1. SteamVR on → start GTA → confirm buildId in log.
2. **H2:** tilt head toward shoulder near a wall/desk — near double-vision from roll should improve vs 243.
3. **H4:** lean / turn head 30° — camera should follow the small position shift (not rotation-only).
4. Compare kill `243` if unsure. Isolate: `900` = H2 only, `901` = H4 only.
