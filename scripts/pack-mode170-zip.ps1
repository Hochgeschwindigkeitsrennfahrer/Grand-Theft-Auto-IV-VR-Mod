# Pack the Mode 170 preset using only explicit project/build inputs.
param(
  [switch] $SkipBuild
)
...
$Stage = Join-Path $Root "dist\gtaiv-dxvk-vr-mode170-stage"
...
@"
gtaiv-dxvk-vr - Mode 170 install
================================

This archive contains only gtaiv-dxvk-vr, its Mode 170 configuration,
...
@"
Mode 170 configuration
======================

The gtaiv_dxvk_vr.* files belong next to GTAIV.exe.
stereo=170 selects fresh left/right DrawScene stereo.
vres=2048 is the conservative starting eye-canvas resolution.
Set stereo=0 to disable stereo. Change only one behavior per headset test and
retain gtaiv_dxvk_vr.log next to GTAIV.exe.
...
- The origin of this software must not be misrepresented; you must not
  ...
  in a product, an acknowledgment in product documentation would be
...
Write-Host "Compressing Mode 170 install pack..."
...
Write-Host "Includes only: Mode 170 ASI/config, verified stock DXVK 3.0.2, OpenVR, and notices."
