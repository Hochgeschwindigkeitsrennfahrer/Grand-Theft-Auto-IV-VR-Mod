...
uint16_t motorLow(uint32_t packed)
{
    return static_cast<uint16_t>(packed & 0xffffu);
}

uint16_t motorHigh(uint32_t packed)
{
    return static_cast<uint16_t>((packed >> 16u) & 0xffffu);
}

uint32_t packMotors(uint16_t left, uint16_t right)
{
    return static_cast<uint32_t>(left)
        | (static_cast<uint32_t>(right) << 16u);
}

bool controllerInputAvailable(
    const PoseBridge& pose,
    uint32_t hand)
{
...
BYTE triggerValue(float value)
{
    return static_cast<BYTE>(std::lround(
        (std::max)(0.0f, (std::min)(value, 1.0f)) * 255.0f));
}

bool synthesizeGamepad(
    const PoseBridge& pose,
    XINPUT_GAMEPAD& gamepad)
{
    ...
    if (haveLeft)
    {
        ...
        if (pressed(left, gtaiv_xr_bridge::ControllerPrimaryClick))
            gamepad.wButtons |= XINPUT_GAMEPAD_X;
        if (pressed(left, gtaiv_xr_bridge::ControllerSecondaryClick))
            gamepad.wButtons |= XINPUT_GAMEPAD_Y;
        if (pressed(left, gtaiv_xr_bridge::ControllerSqueezePressed))
    ...
    if (haveRight)
    {
        ...
        if (pressed(right, gtaiv_xr_bridge::ControllerPrimaryClick))
            gamepad.wButtons |= XINPUT_GAMEPAD_A;
        if (pressed(right, gtaiv_xr_bridge::ControllerSecondaryClick))
            gamepad.wButtons |= XINPUT_GAMEPAD_B;
        if (pressed(right, gtaiv_xr_bridge::ControllerSqueezePressed))
            gamepad.wButtons |= XINPUT_GAMEPAD_RIGHT_SHOULDER;
    }

    const bool leftThumb =
        haveLeft
        && pressed(left, gtaiv_xr_bridge::ControllerThumbstickPressed);
    const bool rightThumb =
        haveRight
        && pressed(right, gtaiv_xr_bridge::ControllerThumbstickPressed);
    const bool menu =
        haveLeft
        && pressed(left, gtaiv_xr_bridge::ControllerMenuClick);
