...
void SetFpFov(int v) {
  ...
  PersistFpFovDegrees(v, static_cast<int>(GetFpRearFovDegrees() + 0.5f),
                      static_cast<int>(GetFpFootFovDegrees() + 0.5f));
}

int GetEyeFwd() {
  return MenuGetFpFootCamForwardCm();
}
void SetEyeFwd(int v) {
  SetFpFootCamForwardCm(v);
  SetEyeForwardCm(v);
}

int GetVehCamFwd() {
  return MenuGetFpVehCamForwardCm();
}
void SetVehCamFwd(int v) {
  SetFpVehCamForwardCm(v);
}

int GetBikeCamFwd() {
  return MenuGetFpBikeCamForwardCm();
}
void SetBikeCamFwd(int v) {
  SetFpBikeCamForwardCm(v);
}

void ActReloadFiles() {
  ReloadStereoMode();
  Log("VrMenu: re-read gtaiv_dxvk_vr.stereo (+ mode enter seeds only if files missing) — mode=%d",
      static_cast<int>(GetStereoMode()));
}

// Real process restart: launch FULL-RESTART.bat next to the ASI, then quit.
void ActFullRestart() {
  char dir[MAX_PATH]{};
  HMODULE self = nullptr;
  if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                              GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                          reinterpret_cast<LPCSTR>(&ActFullRestart), &self) ||
      !self || !GetModuleFileNameA(self, dir, MAX_PATH)) {
    Log("VrMenu: Full restart FAILED (no ASI path)");
    return;
  }
  ...
  if (slash)
    slash[1] = 0;
  char bat[MAX_PATH]{};
  sprintf_s(bat, "%sFULL-RESTART.bat", dir);
  if (GetFileAttributesA(bat) == INVALID_FILE_ATTRIBUTES)
    sprintf_s(bat, "%sgtaiv_dxvk_vr-FULL-RESTART.bat", dir);
  Log("VrMenu: Full restart → ShellExecute %s then TerminateProcess", bat);
  ShellExecuteA(nullptr, "open", bat, nullptr, dir, SW_SHOWNORMAL);
  Sleep(200);
...
const Row kRows[] = {
    ...
    {Kind::Value, "Cam forward (foot)", "On-foot eye past the HEAD bone. Higher = less inside skull.",
     &GetEyeFwd, &SetEyeFwd, 0, 40, 1, "cm", nullptr, nullptr, false, nullptr},
    {Kind::Value, "Cam forward (car)", "In-car eye forward. Separate from bikes.",
     &GetVehCamFwd, &SetVehCamFwd, 0, 40, 1, "cm", nullptr, nullptr, false, nullptr},
    {Kind::Value, "Cam forward (bike)", "Motorcycle/bicycle eye forward (IS_CHAR_ON_ANY_BIKE).",
     &GetBikeCamFwd, &SetBikeCamFwd, 0, 40, 1, "cm", nullptr, nullptr, false, nullptr},
    ...
    {Kind::Action, "Re-read settings files",
     "Re-loads stereo/ipd/scale/fov/camfwd from disk (menu changes already auto-save).", nullptr,
     nullptr, 0, 0, 0, nullptr, nullptr, nullptr, true, &ActReloadFiles},
    {Kind::Action, "Full restart",
     "Quits the game and runs FULL-RESTART.bat (real process restart — not just file re-read).",
     nullptr, nullptr, 0, 0, 0, nullptr, nullptr, nullptr, true, &ActFullRestart},
