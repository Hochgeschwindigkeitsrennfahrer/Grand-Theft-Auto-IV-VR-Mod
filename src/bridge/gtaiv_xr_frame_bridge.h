...
enum FrameFlags : uint32_t
{
    ...
    // seam. The x64 host requires the exact shared capture pose before submit.
    // FirstPersonCamera and NativeHeadHidden are independent observations; the
    // OpenVR Mode 204 submit path never used either one to withhold valid eyes.
    VerifiedParentDualStereo = 1u << 11u,
    // The world pair was rendered by the first-person camera path. This is
    // telemetry, not a request for the host to move a camera.
    FirstPersonCamera = 1u << 12u,
    // GTA's native player-head suppression was operational for this pair.
    // This is telemetry and never blocks an otherwise valid Mode 204 submit.
